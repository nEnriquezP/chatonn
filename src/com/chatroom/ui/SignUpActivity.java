...
public class SignUpActivity {
	...
	public SignUpActivity(ClientModel cm) throws IOException {
		clientModel = cm;
		jFrame = new JFrame("Live Chat Sign Up");
	...
	private void signUp(String username, String password) {
		if(username.equals("Enter Username") || username.length() <= 0) {
			...
			JOptionPane.showMessageDialog(null, "Enter password properly 6 to 10 characters with 1 uppercase, lowercase and one special character.", null, JOptionPane.ERROR_MESSAGE);
		}
		else {
	...
	private void initializeAllWithProperties() {
		...
		jBtnSignUp.setBorder(new LineBorder(Config.colorPrimary, 3));
    ...
    private boolean checkPass(String passCheck) {
       
          // Check length
    if (passCheck.length() < 6 || passCheck.length() > 10) {
        return false;
    }
    ...
    for (char c : passCheck.toCharArray()) {
        ...
        System.out.println(hasLowercase+" "+hasSpecial+" "+hasUppercase);
